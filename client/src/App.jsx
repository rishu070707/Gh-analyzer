...
import GlitchText from './components/GlitchText'; // New interactive component
import Typewriter from './components/Typewriter'; // New interactive component
import TokenModal from './components/TokenModal'; // Rate Limit Handler
...
function App() {
  ...
  // Trigger typewriter again on new search or data changes if desired
  useEffect(() => {
    if (loading) {
  ...
  const handleSearch = async (url, token = null) => {
    ...
    setError('');
    setData(null);
    setPendingUrl(url); // Store URL for retry if needed

    try {
      const result = await analyzeRepo(url, token);
      setData(result.data);
      setTokenModalOpen(false); // Close modal on success
    } catch (err) {
    ...
    <div className="min-h-screen text-gray-300 p-4 md:p-8 font-mono selection:bg-neon-green/30 selection:text-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <header className="text-center mb-16 space-y-4 animate-fade-in pt-10 relative">
          <div className="inline-flex items-center justify-center p-4 bg-black-800 border border-neon-dim/50 rounded-none mb-6 shadow-neon-dim animate-pulse-glow group hover:border-neon-bright hover:shadow-neon-hover transition-all duration-300 cursor-pointer">
          ...
          <h1 className="text-5xl md:text-7xl font-bold text-white tracking-tighter uppercase relative inline-block">
            ...
            <GlitchText text="GH_ANALYZER" className="bg-clip-text text-transparent bg-gradient-to-b from-white to-gray-400 hover:text-white transition-colors cursor-default select-none" />
            <span className="text-neon-bright text-xs md:text-sm font-mono absolute -top-4 -right-8 animate-pulse border border-neon-dim px-1 rounded-sm">v2.0.4</span>
          </h1>
          <div className="h-8 flex items-center justify-center">
            {showTypewriter ? (
              ...
              <p className="text-neon-dim text-lg h-full opacity-0">Initialize...</p>
            )}
          </div>
        </header>
        ...
        {/* Error State */}
        {error && (
          <div className="max-w-2xl mx-auto p-4 bg-red-900/20 border border-red-500 rounded-none flex items-center gap-4 text-red-400 mb-12 animate-fade-in font-mono shadow-[0_0_10px_rgba(239,68,68,0.2)]">
        ...
        {data && (
          <div className="space-y-12 animate-slide-up pb-20">
            ...
            {/* Top Row: Introduction & Persona */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch">

              {/* Repo Header Card */}
              <div className="lg:col-span-2 glass-card flex flex-col justify-between group !border-neon-dim/20 hover:!border-neon-bright/40">
                <div className="absolute top-0 right-0 p-2 opacity-50">
                  <Activity className="w-24 h-24 text-neon-dim/10" />
                </div>

                <div className="flex flex-col md:flex-row items-start md:items-center gap-8 mb-8 relative z-10">
                  <img
                  ...
                  <div className="flex-1 space-y-2">
                    <div className="flex items-center gap-3 flex-wrap">
                      <span className="px-2 py-1 bg-neon-dim/10 text-neon-bright border border-neon-dim/40 text-xs font-bold uppercase tracking-widest">
                        {data.summary.language || 'UNKNOWN'}
                      </span>
                      <a href={data.summary.owner.html_url} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-white transition-colors">
                        <ExternalLink className="w-4 h-4" />
                      </a>
                    </div>
                    <h2 className="text-4xl font-bold text-white leading-none tracking-tighter truncate w-full">
                      {data.summary.name}
                    </h2>
                    <p className="text-gray-400 font-mono text-sm leading-relaxed max-w-2xl border-l-2 border-neon-dim/30 pl-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-6 border-t border-white/5 relative z-10">
                  <div className="text-center p-2 border border-white/5 bg-black-900/50 hover:border-neon-dim/50 transition-colors">
                  ...
                  <div className="text-center p-2 border border-white/5 bg-black-900/50 hover:border-neon-dim/50 transition-colors">
                    <Clock className="w-4 h-4 text-white/70 mx-auto mb-2" />
                    <div className="text-xs font-bold text-white truncate px-1 pt-2">
                      {new Date(data.summary.updated_at).toLocaleDateString()}
                    </div>
                    <div className="text-[10px] text-gray-500 uppercase tracking-widest mt-1">Last Update</div>
                  </div>
                  <div className="text-center p-2 border border-white/5 bg-black-900/50 hover:border-neon-dim/50 transition-colors">
                    ...
                    <div className="text-[10px] text-gray-500 uppercase tracking-widest">Issues</div>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
              ...
              <div className="lg:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <StatCard
                  ...
                  icon={<Activity className={`w-6 h-6 ${data.health.calculated_bus_factor < 2 ? 'text-red-500' : 'text-neon-bright'}`} />}
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
              <div className="xl:col-span-2 space-y-8">
                <h2 className="text-2xl font-bold text-white uppercase tracking-widest border-l-4 border-neon-bright pl-4 mb-8">
